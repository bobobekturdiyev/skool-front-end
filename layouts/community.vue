<template>
  <header class="pb-[60px] relative">
    ...
    <div class="pb-[40px] mx-auto xl:px-[200px] lg:px-[100px] md:px-[100px] sm:px-[50px] px-5 max-w-[1536px]">
      <div v-if="useGroup.store.group_by_username.status == 'active'" :class="isLoading.isLoadingType('getByUsername') ? 'pointer-events-none' : ''
        ">
        <Community_tabs />
      </div>
      <slot />
    ...
    <!-- cropper image -->
    <el-dialog v-model="isLoading.store.inviteModal" width="1036px" align-center
      class="bg-opacity-50 !p-0 !rounded-lg members_dialog overflow-hidden">
      <MembersModal />
    </el-dialog>

    <!-- add course -->
    <el-dialog v-model="useClassroom.store.add_course" align-center
      ...
      <form @submit.prevent="handleSubmit" class="space-y-6">
        <h1 class="md:text-2xl text-lg pb-2 font-semibold _c07">
          <span v-if="useClassroom.store.edit_course">Edit</span><span v-else>Add</span> course
        </h1>
        <div>
          <input @input="handleInput('input')" v-model="useClassroom.create.title" type="text" class="text-sm"
        ...
        <div>
          <textarea @input="handleInput('textarea')" id="write_message" v-model="useClassroom.create.description"
        ...
        <div>
          <label class="_ca1 text-xs" for="access">Who can access this course</label>
          <el-select class="block w-full mt-2" v-model="useClassroom.create.access" placeholder="Select">
            <el-option v-for="item in access_list" :key="item.value" :label="item.label" :value="item.value"
              :disabled="item.disabled">
              <div class="flex items-center gap-2">
                {{ item.label }}
                <img v-if="useClassroom.create.access == item.value" src="@/assets/svg/checked.svg" alt="" />
              </div>
            </el-option>
          </el-select>
        </div>
        <div v-if="useClassroom.create.access == 'level_lock'">
          ...
          <el-select class="block w-full mt-2 el_select" v-model="useClassroom.create.level" placeholder="Select">
            <el-option v-for="item in 9" :key="item" :label="item" :value="item">
              <div class="flex items-center gap-2">
                {{ item }}
                <img v-if="useClassroom.create.level == item" src="@/assets/svg/checked.svg" alt="" />
              </div>
            </el-option>
          </el-select>
        </div>
        <div class="sm:flex gap-4">
          <label v-if="!isLoading.store.croppedImage" for="add_photo_course"
          ...
          <label class="relative imagelabel" v-else for="add_photo_course">
            ...
            <img class="sm:h-[188px] h-[164px] sm:w-[366px] w-full overflow-hidden rounded-xl object-cover"
          ...
          <div class="sm:block flex items-center justify-between py-5">
            ...
            <label for="add_photo_course"
              class="_ca1 full_flex uppercase block h-10 px-6 text-sm font-semibold border border-[#E0E0E0] r_8">
              change
            </label>
          </div>
          <input @change="handleAddedPhoto" id="add_photo_course" type="file" class="w-0 h-0 overflow-hidden !p-0" />
        </div>
        <div class="flex items-center justify-between !sm:mt-6 !mt-1"
          :class="useClassroom.store.add_course ? '' : 'pb-12'">
          <div class="flex items-center gap-6">
            <p v-if="useClassroom.create.published" class="text-[16px] _c07 font-medium">
              Published
            </p>
            <p v-else class="text-[16px] font-medium">Draft</p>
            <el-switch v-model="useClassroom.create.published" class="ml-2" />
          </div>
          <div v-if="!useClassroom.store.edit_course" class="flex gap-3 text-sm font-semibold">
            <button type="button" @click="useClassroom.store.add_course = false"
              class="uppercase h-10 px-6 rounded-lg _ca1">
              cancel
            </button>
            <button :type="isLoading.isLoadingType('createCourse') ? 'button' : 'submit'
        " :class="useClassroom.create.title ? 'b_cbc _c07' : 'b_ce0 _ca1'" @click="reposrtToAdmins"
              class="uppercase h-10 px-6 rounded-lg" v-loading="isLoading.isLoadingType('createCourse')">
              add
            </button>
          </div>
        </div>
        <div v-if="useClassroom.store.edit_course"
    ...
    <!-- Delete -->
    <el-dialog v-model="useClassroom.store.delete" width="400" align-center
      ...
      <div class="space-y-7">
        ...
        <div class="flex justify-end gap-3 text-sm font-semibold">
          <button @click="useClassroom.store.delete = false" class="uppercase h-10 px-6 rounded-lg _ca1">
            cancel
          </button>
          <button @click="useClassroom.delete_course" v-loading="isLoading.isLoadingType('deleteCourse')"
...
watch(() => useClassroom.store.add_course, () => {
