<template>
  <section>
    <div class="grid grid-cols-3 gap-6 justify-between my-8 mt-[18px]">
      <div v-if="isLoading.isLoadingType('getClassrooms')" v-for="i in 10"
        class="w-full bg_loading animate-pulse h-[400px] r_16 overflow-hidden">
        <div class="w-full card md:h-[180px] sm:h-[140px] h-[120px] object-cover bg-gray-600"></div>
      </div>
      <div v-else v-for="i in useClassroom.store.classrooms" data-aos="zoom-in"
        class="w-full course_card relative bg-white r_16 overflow-hidden">
        <div :class="i.id == store.activeEdit ? 'edit_button_active' : ''"
          ...
          <el-dropdown placement="bottom-end" class="dropdown" trigger="click">
            <img @click="store.activeEdit = i.id" class="w-5 h-5" src="@/assets/svg/three_dot.svg" alt="" />
            <template #dropdown>
              <el-dropdown-menu @click="store.activeEdit = ''" class="community_dropdown min-w-[200px] dropdown_shadow">
                ...
                <el-dropdown-item @click="useClassroom.delete_course(i.title)">Delete course</el-dropdown-item>
              </el-dropdown-menu>
            </template>
          </el-dropdown>
        </div>
        <div>
          <img v-if="i.image" class="w-full card md:h-[180px] sm:h-[140px] h-[120px] object-cover" :src="i.image"
            alt="" />
      ...
      <div @click="useClassroom.store.add_course = true"
    ...
    <!-- add course -->
    <el-dialog v-model="useClassroom.store.add_course" width="780" align-center class="bg-opacity-50 p-6 !rounded-lg">
      <form @submit.prevent="handleSubmit" class="space-y-6">
        <h1 class="text-2xl pb-2 font-semibold _c07"><span v-if="useClassroom.store.edit_course">Edit</span><span
            v-else>Add</span> course</h1>
        <div>
        ...
        <div class="flex items-center justify-between" :class="useClassroom.store.add_course ? '' : 'pb-12'">
          <div class="flex items-center gap-6">
            <p v-if="useClassroom.create.published" class="text-[16px] font-medium">
              Published
            </p>
        ...
        <div v-if="useClassroom.store.edit_course"
          ...
          <button v-loading="isLoading.isLoadingType('deleteEvent')" type="button"
