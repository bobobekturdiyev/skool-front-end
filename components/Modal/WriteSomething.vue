<template>
  <section>
    ...
    <div v-if="usePost.store.writingModal" @click="usePost.store.writingModal = false"
      class="fixed top-0 bg-black bg-opacity-50 min-h-screen w-full z-50 left-0"></div>
    <form @submit.prevent="addVideo.handleSubmit" v-if="usePost.store.writingModal"
      ...
      <div class="flex md:items-center gap-3 b_cf0f md:h-[52px] h-[64px] px-5 py-3">
        ...
        <p class="text-sm flex flex-wrap items-start gap-1 leading-4">
          <span class="font-semibold">{{ isLoading.user_update_checker.name }}
            {{ isLoading.user_update_checker.surname }}</span>
          <span class="_ca1">posting in</span>
          <span class="font-semibold _c2a md:w-auto w-full">{{ useGroup.store.group_by_username.name }}</span>
        </p>
      </div>
      <div class="md:p-5 p-3 space-y-5 bg-white">
        ...
        <div v-if="Object.keys(usePost.store.polls).length" class="border_ce0 rounded-lg p-4">
          <div class="flex items-center justify-between">
            ...
            <p @click="usePost.store.polls = {}" class="font-medium cursor-pointer hover:underline text-sm _ca1">
              Remove
            </p>
          </div>
          <ul class="space-y-2 mt-2">
            <li v-for="(i, index) in Object.keys(usePost.store.polls)" class="flex items-center gap-4">
              <input v-model="usePost.store.polls[i]" type="text" :placeholder="`Option ${index + 1}`" />
              <img v-if="Object.keys(usePost.store.polls).length > 2" @click="addVideo.deletePoll(i)"
                class="opacity-50 hover:bg-gray-300 cursor-pointer rounded-full p-2" src="@/assets/svg/x.svg" alt="" />
            </li>
          </ul>
          <button @click="addVideo.addPoll" type="button" class="uppercase border_ce0 px-3 rounded-md mt-2" :class="Object.keys(usePost.store.polls).length >= 10 ? 'b_ce0 _ca1' : ''
      ">
            Add option
          </button>
        </div>
        <ul v-if="addVideo.store.files.length" class="flex gap-5 overflow-x-auto">
          <draggable :list="addVideo.store.files" group="grid" :animation="200"
            ...
            <li class="relative imagelabel" v-for="(i, index) in addVideo.store.files">
              ...
              <img v-if="i.type == 'image'" class="w-40 h-40 min-w-[160px] border rounded-xl object-cover" :src="i.url"
                alt="" />
      ...
      <div class="2xl:flex items-center justify-between 2xl:space-y-0 space-y-5 p-5 pt-0 2xl:mt-0 -mt-10 bg-white">
        <div class="textarea_icon flex items-center">
          <label for="add_image" class="icon full_flex h-10 w-10 hover:bg-[#F2F2F2] r_8">
          ...
          <div @click="addVideo.openPoll" class="icon full_flex h-10 w-10 cursor-pointer hover:bg-[#F2F2F2] r_8">
            <img src="@/assets/svg/textarea/poll.svg" alt="" />
          </div>
          <el-dropdown placement="bottom-end" class="dropdown" trigger="click">
            <div id="emojidrop1" class="icon full_flex h-10 w-10 relative  hover:bg-[#F2F2F2] r_8">
      ...
      usePost.store.category_id
        ...
        <div class="flex gap-3 font-semibold md:text-[16px] text-sm">
          <button @click="usePost.store.writingModal = false" class="uppercase h-10 px-6 rounded-lg _ca1">
            cancel
          </button>
          <button v-loading="isLoading.isLoadingType('writePost')"
