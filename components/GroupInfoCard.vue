  ...
  <main>
    ...
    <section v-else class="min-w-[280px] md:max-w-[280px]">
      <div class="rounded-2xl overflow-hidden bg-white">
        <img class="w-full h-[150px] object-cover" v-if="useGroup.store.group_by_username?.image"
          :src="useGroup.store.group_by_username?.image" alt="" />
        <div v-else-if="role_ac.includes(useGroup.store.group_by_username.type)" @click="() => {
      ...
      " class="full_flex hover:underline cursor-pointer w-full h-[150px] text-white"
        ...
        <div v-else class="full_flex hover:underline cursor-pointer w-full h-[150px] text-white"
          :style="`background: ${useGroup.store.group_by_username?.color}`"></div>
      ...
      useGroup.store.group_by_username?.group_type == "private"
        ...
        useGroup.store.group_by_username?.excerpt
          ...
          <ul v-if="useGroup.store.showLinksPublic" class="py-1 text-sm border-y border-[#E0E0E0]">
            <li v-for="link in useGroup.store.group_by_username.links">
              <a :href="link.url" target="_blank" v-if="link.is_public == 0"
                class="flex items-center gap-1 h-7 px-3 _c59">
                <img src="@/assets/svg/href.svg" alt="" />
                {{ link.label }}
              </a>
              <a v-else-if="useGroup.store.group_by_username.status == 'active'" :href="link.url" target="_blank"
                v-if="link.is_public" class="flex items-center gap-1 h-7 px-3 _c59">
                <img src="@/assets/svg/href.svg" alt="" />
                {{ link.label }}
              </a>
            </li>
          ...
          <div v-if="useGroup.store.group_by_username.status == 'pending'">
            <button class="b_cf2 rounded-lg pointer-events-none w-full font-semibold text-sm">
              MEMBERSHIP PENDING
            </button>
            <button @click="useMember.joinToGroup" v-loading="isLoading.isLoadingType('joinGroup')"
              class="_ca1 hover:underline text-[10px] text-center w-full">
              Cancel membership request
            </button>
          </div>
          <button v-else-if="useGroup.store.group_by_username.status == 'active'"
          ...
          <div v-else-if="useGroup.store.group_by_username.status == 'banned'">
            <button class="b_cf2 _ca1 rounded-lg pointer-events-none w-full font-semibold text-sm">
              BANNED
            </button>
          ...
          <button v-else @click="joinToGroup" v-loading="isLoading.isLoadingType('joinGroup')"
      ...
      <div class="bg-white mt-5 r_16 px-4 py-5" v-if="useMember.store.activities30?.length">
        <h1 class="font-semibold">Leaderboard (30-day)</h1>
        <div class="text-sm border-t borer-[#E0E0E0] py-4 mt-4">
          <div v-for="(i, index) in useMember.store.activities30" class="flex items-center h-[44px] justify-between">
      ...
      " class="full_flex _c07 font-medium rounded-full w-7 h-7 min-w-[28px]">
              ...
              <img class="h-5 w-5 min-w-[20px] rounded-full object-cover" :src="i.user.image" title="user" />
              <p class="truncate font-semibold w-[70%] text-start">
                {{ i.user.name }} {{ i.user.surname }}
              </p>
            </div>
            <p class="_c2a font-medium text-xs min-w-[42px] text-end">+{{ i.like_count }}</p>
          </div>
        </div>
        <router-link class="_c2a text-sm font-semibold" :to="`/${$router.currentRoute.value.params.community}/leaderboards`">See all leaderboards</router-link>
      </div>
      <div v-if="$router.currentRoute.value.name == 'community-about'" class="full_flex mt-8 gap-2 leading-[18px]">
        <p>Powered by</p>
    ...
    <!-- Membership pending -->
    <el-dialog v-model="useGroup.modal.pending" width="400" align-center class="!rounded-xl overflow-hidden px-6 py-7">
      <div class="space-y-7 _c07 text-center">
        ...
        <div class="flex justify-end gap-3 text-sm font-semibold">
          <button @click="useGroup.modal.pending = false" v-loading="isLoading.isLoadingType('deletePost')"
