<template>
  <main>
    <section class="md:static sticky -top-6 z-20">
      <div class="md:space-y-5 space-y-3 text-center md:mt-[72px] mt-8 font-semibold text-xl">
        <h1 class="md:text-[45px] text-2xl">Discover communities</h1>
        <p>
          or
          <span class="_c2a cursor-pointer hover:opacity-70">create your own</span>
        </p>
      </div>
      <div class="xl:relative !sticky top-40 md:w-[616px] mx-auto mt-8 md:mb-[52px] mb-6">
        <img class="md:my-[18px] my-[14px] w-5 h-5 left-5 absolute" src="@/assets/svg/search.svg" alt="" />
        <input @input="handleSearch" v-model="useGroup.store.filter.search" autofocus
          class="md:h-[56px] h-12 !pl-[60px] b_none font-medium w-full !rounded-lg" type="text"
    ...
    <section class="md:text-sm text-xs whitespace-nowrap">
      <div class="flex items-start justify-between gap-3">
        <div :class="store.is_show ? 'flex-wrap' : ''"
          class="relative flex gap-3 md:h-auto category_wrap w-full h-[80px] overflow-hidden">
          <button @click="handleCategory('all')" data-aos="zoom-in"
          ...
          <button v-if="isLoading.isLoadingType('groupCategories')" v-for="i in 10"
            class="flex items-center md:h-9 h-8 w-28 animate-pulse hover:bg-[#BCDEFF] hover:bg-opacity-30 duration-700 gap-1 py-2 px-3 rounded-full bg-gray-900"></button>
          <button v-else-if="useCategory.store.categories" @click="handleCategory(i.id)"
            v-for="i in useCategory.store.categories" data-aos="zoom-in" :class="useGroup.store.filter.category_id == i.id ? 'b_cbc' : 'bg-white'
              "
          ...
          <img v-if="!store.is_show && useCategory.store.categories" class="absolute md:block hidden -right-[2px]"
            src="@/assets/svg/shadow_hidden.svg" alt="" />
          <button @click="store.is_show = false" v-if="store.is_show && useCategory.store.categories?.length"
        ...
        <button v-if="!store.is_show && useCategory.store.categories?.length" @click="store.is_show = true"
          class="py-2 px-3 md:h-9 h-8 md:block hidden rounded-full min-w-fit bg-white hover:bg-[#BCDEFF] hover:bg-opacity-30 duration-700">
          More...
        </button>
        <div class="min-w-fit text-end">
          <el-dropdown placement="bottom-start" trigger="click" :hide-on-click="false" class="min-w-fit filter_btn">
            <button class="flex items-center gap-1 py-2 px-3 md:h-9 h-8 rounded-full bg-white">
              <img class="md:h-5 md:w-5 h-4 w-4" src="@/assets/svg/filter.svg" alt="" />
              ...
              <el-dropdown-menu class="flex min-w-[328px] filter_dropdown dropdown_shadow 2xl:!-ml-[60px]">
                <div class="w-[150px] border-r border-[#E0E0E0]">
                  ...
                  <el-dropdown-item class="flex items-center _c07 !px-8 font-medium gap-3 h-[44px] !text-xs">
                    <input @change="onFilterType" id="type_all" class="rounded-full" type="radio" name="type" />
                    <label for="type_all">All</label>
                  </el-dropdown-item>
                  <el-dropdown-item class="flex items-center _c07 !px-8 font-medium gap-3 h-[44px] !text-xs">
                    <input @change="onFilterType" id="public" class="rounded-full" type="radio" name="type" />
                    <label for="public">Public</label>
                  </el-dropdown-item>
                  <el-dropdown-item class="flex items-center _c07 !px-8 font-medium gap-3 h-[44px] !text-xs">
                    <input @change="onFilterType" id="private" class="rounded-full" type="radio" name="type" />
                ...
                <div class="w-[150px]">
                  ...
                  <el-dropdown-item class="flex items-center _c07 !px-8 font-medium gap-3 h-[44px] !text-xs">
                    <input @change="onFilterPrice" id="price_all" class="rounded-full" type="radio" name="price" />
                    <label for="price_all">All</label>
                  </el-dropdown-item>
                  <el-dropdown-item class="flex items-center _c07 !px-8 font-medium gap-3 h-[44px] !text-xs">
                    <input @change="onFilterPrice" id="free" class="rounded-full" type="radio" name="price" />
                    <label for="free">Free</label>
                  </el-dropdown-item>
                  <el-dropdown-item class="flex items-center _c07 !px-8 font-medium gap-3 h-[44px] !text-xs">
                    <input @change="onFilterPrice" id="paid" class="rounded-full" type="radio" name="price" />
          ...
          <button v-if="useCategory.store.categories" @click="store.drawer = true"
    ...
    <!-- cards -->
    <section class="grid md:grid-cols-3 grid-cols-2 md:gap-6 gap-5 justify-between md:mt-7 mt-6">
      <Card />
    </section>
    ...
    <el-drawer v-model="store.drawer" height="400" direction="btt"
      ...
      <div class="flex gap-3 whitespace-nowrap flex-wrap items-center">
        <button @click="handleCategory('all')" :class="useGroup.store.filter.category_id ? 'bg-[#F0F5FA]' : 'b_cbc'"
          class="px-3 rounded-full md:h-9 h-8 hover:bg-[#F0F5FA] hover:bg-opacity-30 duration-700">
          All
        </button>
        <button @click="handleCategory(i.id)" store.drawer="false" v-for="i in useCategory.store.categories" :class="useGroup.store.filter.category_id == i.id ? 'b_cbc' : 'bg-[#F0F5FA]'
