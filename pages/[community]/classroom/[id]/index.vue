  ...
  <main class="flex gap-6">
            ...
            <div class="flex items-center h-[44px] r_8 cursor-pointer">
              <div @click="handleActive(i.id)" :class="useClassroom.local_store.activeName == i.id
                ...
                <div class="flex items-center gap-[10px]">
                  ...
                  <img :class="useClassroom.local_store.activeName == i.id ? 'rotate-180' : ''" class="duration-500"
                    src="@/assets/svg/select_arrow.svg" alt="" />
                </div>
              </div>
            </div>
            <ul :class="useClassroom.local_store.activeName == i.id ? '' : 'h-0 overflow-hidden'"
    ...
    <section class="w-full">
      ...
      <form @submit.prevent="handleSubmit" v-else class="bg-white p-6 r_16">
        <div>
          <input @input="handleInput('input')" v-model="useClassroom.module.name" type="text" placeholder="Title"
            class="text-xl h-12 font-semibold" required />
          <p class="text-end mt-2 _ca1 text-sm">
            {{ useClassroom.module.name?.length }}/50
          </p>
        </div>
        <div v-if="!useClassroom.module.video" @click="useClassroom.local_store.addVideoModal = true"
        ...
        <div class="flex items-center justify-between">
          <div class="full_flex gap-6 _c07">
            <button type="button" class="full_flex gap-2 text-sm border border-[#E0E0E0] rounded-[4px] px-3">
    ...
    <!-- add Video link -->
    <el-dialog v-model="useClassroom.local_store.addVideoModal" width="400" align-center
      ...
      <form @submit.prevent="handleVideoLink">
        ...
        <input v-model="useClassroom.local_store.videoLink" class="text-sm" type="text" placeholder="Link" />
        ...
        <div class="flex justify-end gap-3 mt-7 text-sm font-semibold">
          <button @click="useClassroom.local_store.addVideoModal = false" type="button"
            class="uppercase h-10 px-6 rounded-lg _ca1">
            cancel
          </button>
          <button :class="useClassroom.local_store.videoLink ? 'b_cbc _c07' : 'b_ce0 _ca1'"
            class="uppercase h-10 px-6 rounded-lg" v-loading="isLoading.isLoadingType('createModule')">
            save
          </button>
        </div>
