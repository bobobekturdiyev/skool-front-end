...
<main class="sm:-mt-2 -mt-5 -mb-[80px]">
    <nav v-show="store.is_open" @click="store.is_open = false" class="sm:hidden flex items-center cursor-pointer -mt-10 mb-[10px] h-[60px] px-5">
    ...
    <div class="flex gap-6">
        <aside :class="store.is_open ? 'sm:block hidden' : 'min-w-full'" class="md:min-w-[280px] sm:min-w-[200px] h-[calc(100vh_-140px)] overflow-hidden overflow-y-auto !duration-300">
            <ul class="_c07 text-sm">
                <li @click="openData(index, i)" :class="store.slideStep == index ? '_c2a font-medium bg-[#F0F5FA]' : 'bg-white'" class="flex gap-4 items-center cursor-pointer r_8 overflow-hidden" v-for="(i, index) in settings_sidebar">
                    <p :class="store.slideStep == index ? 'b_c2a' : ''" class="w-1 h-[44px]"></p>
                    <p>{{ i }}</p>
                </li>
            </ul>
        </aside>
        <div class="w-full bg-white r_16 h-[calc(100vh_-140px)] overflow-hidden profile_accordion" :class="store.is_open ? '' : 'sm:block hidden'">
            <div class="mainSlider h-[calc(100vh_-140px)] duration-700">
                <form @submit.prevent="useSettings.updateUserData()" class="h-[calc(100vh_-140px)] overflow-hidden overflow-y-auto text-sm _c07 p-5 w-full">
                    ...
                    <div class="space-y-6">
                        <div v-loading="isLoading.isLoadingType('updateUserData') || isLoading.isLoadingType('getUserData')" class="flex items-center md:gap-8 gap-4 mt-6">
                            <img class="h-[100px] w-[100px] rounded-full object-cover" :src="isLoading.user.image" alt="">
                            <label for="profile_img" class="md:text-[16px] text-sm border_cbc lg:px-4 sm:px-2 px-4 md:py-3 py-2 _c2a r_8 font-semibold text-center uppercase block full_flex">change
                                profile
                                photo
                            </label>
                        ...
                        <div>
                            <div class="grid grid-cols-2 gap-6">
                                <input disabled v-model="isLoading.user.name" class="text-sm _ca1 placeholder-[#A1A1A1]" type="text" placeholder="Name">
                                <input disabled v-model="isLoading.user.surname" class="text-sm _ca1 placeholder-[#A1A1A1]" type="text" placeholder="Surname">
                            </div>
                        ...
                        <div>
                            ...
                            <input v-model="isLoading.user_update_checker.username" disabled id="url" class="text-sm _ca1 placeholder-[#A1A1A1]" type="text">
                        ...
                        <div>
                            ...
                            <textarea @input="listeneerUserData" v-model="isLoading.user_update_checker.bio" id="bio" class="h-[100px] w-full rounded-[4px] text-sm _c07 placeholder-[#A1A1A1]" placeholder="Description"></textarea>
                        ...
                        <div class="grid grid-cols-2 gap-6">
                            <div>
                                ...
                                <input @input="listeneerUserData" v-model="isLoading.user_update_checker.location" id="location" class="text-sm _c07 placeholder-[#A1A1A1]" type="text">
                            </div>
                            <div>
                                <label class="block _ca1 text-xs mb-2" for="griggs">Myers Briggs</label>
                                <el-select @change="listeneerUserData" v-model="isLoading.user_update_checker.myers_briggs" class="w-full">
                                    <el-option v-for="item in myers_briggs" :key="item" :label="item" :value="item">
                                        <div class="flex items-center gap-2">
                                            {{ item }}
                                            <img v-if="isLoading.user_update_checker?.myers_briggs == item" src="@/assets/svg/checked.svg" alt="" />
                                        </div>
                                    </el-option>
                                </el-select>
                            </div>
                        ...
                        <el-collapse accordion class="space-y-6">
                            <el-collapse-item name="1">
                                ...
                                <div class="space-y-6 mt-6">
                                    <input @input="listeneerUserData" v-model="isLoading.user_update_checker.socials[i]" v-for="i in Object.keys(isLoading.user_update_checker?.socials)" type="url" :placeholder="i">
                                </div>
                            </el-collapse-item>
                            <el-collapse-item title="Feedback" name="2">
                                ...
                                <div class="space-y-6 mt-6">
                                    ...
                                    <div class="space-y-10 mt-6">
                                        <div v-for="i in isLoading.user.creator" class="flex items-center justify-between h-10">
                                            <div class="full_flex gap-4">
                                                <div class="w-10 h-10 b_c2a r_8 full_flex overflow-hidden" v-if="true">
                                                    <p v-if="!i.image" class="font-semibold text-white">{{
            createLogo(i.name) }}</p>
                                                    <img v-else class="w-10 h-10 object-cover" :src="i.image" alt="">
                                                </div>
                                    ...
                                    <div class="space-y-10 mt-6">
                                        <div v-for="i in isLoading.user.member" class="flex items-center justify-between h-10">
                                            <div class="full_flex gap-4">
                                                <div class="w-10 h-10 b_c2a r_8 full_flex overflow-hidden" v-if="true">
                                                    <p v-if="!i.image" class="font-semibold text-white">
                                                        {{ createLogo(i.name) }}
                        ...
                        <button v-loading="isLoading.isLoadingType('updateUserData')" :type="useSettings.store.is_update ? 'submit' : 'button'" :class="useSettings.store.is_update ? 'b_cbc _c07' : 'b_ce0 _ca1'" class="font-semibold px-6 r_8 mt-6 uppercase">update profile</button>
                    </div>
                ...
                <section class="h-[calc(100vh_-140px)] overflow-hidden overflow-y-auto text-sm _c07 p-5 w-full space-y-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="flex gap-4">
                            ...
                            <button class="full_flex gap-1 border border_cbc r_8 _c2a px-4 h-10 uppercase whitespace-nowrap font-semibold">
                                change email
                            </button>
                        </div>
                    </div>
                    ...
                    <div class="w-full timezone">
                        <el-select @change="changedTimeZone" v-model="isLoading.user_update_checker.location" filterable class="!w-full" placeholder="(GMT +05:00) Asia/Tashkent">
                            <el-option v-for="item in timeZones" :key="item" :label="item" :value="item">
                                <div class="flex items-center gap-2">
                                    {{ item }}
                                    <img v-if="useSettings.store.timezone == item" src="@/assets/svg/checked.svg" alt="" />
                                </div>
                            </el-option>
                        </el-select>
                    </div>
                    <button v-loading="isLoading.isLoadingType('updateUserData')" @click="updateTimeZone" :class="store.updateTimeZone ? '_c07 b_cbc' : '_ca1 b_ce0'" class=" font-semibold px-6 r_8 mt-6 uppercase">update
                        account</button>
                    <h1 class="text-xl font-semibold">Sessions</h1>
                    <button class="full_flex gap-1 border border_ce0 r_8 _ca1 px-4 h-10 uppercase whitespace-nowrap font-semibold">log
                        out everywhere</button>
                </section>
                <form @submit.prevent="useSettings.changePassword" class="h-[calc(100vh_-140px)] overflow-hidden overflow-y-auto text-sm _c07 p-5 w-full space-y-6">
                    ...
                    <FloatingInput id="old_password" type="password" v-model="useSettings.changepassword.old_password" @input="listenerChangePassword" label="Old password" required />
                    ...
                    <FloatingInput id="new_password" type="password" v-model="useSettings.changepassword.password" @input="listenerChangePassword" label="New password" required />
                    ...
                    <FloatingInput id="confirm_new_password" type="password" v-model="useSettings.changepassword.password_confirmation" @input="listenerChangePassword" label="Confirm new  password" required />
                    ...
                    <button :type="useSettings.store.is_changepass ? 'submit' : 'button'" :class="useSettings.store.is_changepass ? 'b_cbc _c07' : 'b_ce0 _ca1'" class=" font-semibold px-6 r_8 mt-6 uppercase">change
                        password</button>
                </form>
                <section class="h-[calc(100vh_-140px)] overflow-hidden overflow-y-auto text-sm _c07 p-5 w-full space-y-6">
                                ...
                                <div class="cursor-grab" v-for="i in useGroup.store.mygroups">
                                    <div class="flex items-center justify-between">
                                        <div class="full_flex items-start gap-4">
                                            <img class="!w-10 !h-10 min-w-[40px] object-cover r_8 !overflow-hidden" v-if="i.icon" :src="i.icon" alt="" />
                                            <div v-else class="full_flex uppercase w-10 h-10 min-w-[40px] r_8" :style="`background: ${i.color}`">
                                                {{ i.initials }}
                                            </div>
                                            <div>
                                                <h1 class="font-semibold">{{ i.name }}</h1>
                                                <p><span class="capitalize">{{ i.group_type }}</span> • <span class="capitalize">
                                                        {{i.group_price }}
                                                    </span></p>
                                            </div>
                                        </div>
                                        <div class="flex items-center gap-4">
                                            <button @click="() => routeToGroupSettings(i.username)" class="xl:flex hidden items-center justify-center gap-[10px] border border_cbc r_8 _c2a px-4 h-10 uppercase whitespace-nowrap font-semibold">
                                                {{ i.type }}
                                                <img src="@/assets/svg/settings/settings.svg" alt="">
                                            </button>
                                            <div @click="useGroup.pinGroupPinned(i)" class="flex items-center gap-4 cursor-pointer">
                                    ...
                                    <div class="md:flex grid grid-cols-2 gap-4 md:!mt-4 !mt-[10px]">
                                        <button @click="() => routeToGroupSettings(i.username)" class="xl:hidden flex items-center justify-center gap-[10px] border border_cbc r_8 _c2a px-4 h-10 uppercase whitespace-nowrap font-semibold">
                                            {{ i.type }}
                                            <img src="@/assets/svg/settings/settings.svg" alt="">
                                        </button>
                                    </div>
                ...
                <section class="h-[calc(100vh_-140px)] animate-left overflow-hidden overflow-y-auto text-sm _c07 p-5 w-full">
                    <div class="space-y-6 r_8">
                        ...
                        <p>
                            Cards for group memberships show here. Cards for groups you own are <span class="_c2a">here.</span>
                        </p>
                    ...
                    <button @click="useSettings.store.addCartModal = true" class="_c07 font-semibold px-6 r_8 mt-6 uppercase b_cbc">add
                        card</button>
                </section>
                <section class="h-[calc(100vh_-140px)] animate-left overflow-hidden overflow-y-auto text-sm _c07 p-5 w-full">
                    <div class="space-y-6 r_8">
                        ...
                        <p>
                            Receipts for group memberships show here. Receipts for groups you own are <span class="_c2a">here.</span>
                        </p>
                        <div class="full_flex flex-col gap-4 _c07 h-[180px] r_8">
                            <img src="@/assets/svg/settings/nocard.svg" alt="">
    ...
    <el-dialog v-model="useSettings.store.addCartModal" width="400" align-center class="bg-opacity-50 !rounded-lg">
        ...
        <form>
            ...
            <div class="flex justify-end mt-4">
                <button @click="useSettings.store.addCartModal = false" class="_ca1 font-semibold px-6 r_8 uppercase">cancel</button>
    ...
    <el-dialog v-model="useSettings.store.editNameModal" width="400" align-center class="bg-opacity-50 !rounded-lg">
        ...
        <form @submit.prevent="updateuserName">
            <div>
                ...
                <input v-model="store.user_name.name" id="name" class="placeholder-[#A1A1A1]" type="text" placeholder="" required>
            </div>
            <div>
                <label for="surname" class="_ca1 mb-2 block mt-5">Last name</label>
                <input v-model="store.user_name.surname" id="surname" class="placeholder-[#A1A1A1]" type="text" placeholder="" required>
            </div>

            <div class="flex justify-end mt-4">
                <button @click="useSettings.store.editNameModal = false" type="button" class="_ca1 font-semibold px-6 r_8 uppercase">cancel</button>
                <button v-loading="isLoading.isLoadingType('updateUserData')" v-if="isLoading.user_update_checker.name != store.user_name.name || isLoading.user_update_checker.surname != store.user_name.surname" class="b_cbc _c07 font-semibold px-6 r_8 uppercase">save</button>
                <button v-loading="isLoading.isLoadingType('updateUserData')" type="button" v-else class="b_ce0 _ca1 font-semibold px-6 r_8 uppercase">save</button>
            </div>
        </form>
    </el-dialog>

    <!-- cropper image -->
    <el-dialog v-model="isLoading.store.cropModal" v-if="isLoading.store.cropModal" width="780" align-center class="bg-opacity-50 p-6 !w-[400px] !rounded-lg">
        <cropper-image />
